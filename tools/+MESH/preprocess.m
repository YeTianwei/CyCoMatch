...
%       - IfFindConnectivity: if compute the adjacency properties
...
if exist(save_filename,'file')
    ...
    if param.IfComputeNormals
        [S.normals_vtx, S.normals_face] = MESH.compute_vtx_and_face_normals(S);
    end
    
    % compute the adjacency properties 
    if param.IfFindConnectivity
        S.connectivity = connectivity(S.surface.TRIV);
    end
...
default_param.IfComputeLB = false;
default_param.IfComputeGeoDist = false;
if S.nv > 5000
    default_param.IfComputeGeoDist = false;
end
default_param.IfComputeNormals = false;
default_param.IfFindEdge = false;
default_param.IfFindNeigh = false;
default_param.MeshName = S.name;
default_param.IfFindConnectivity = false;
end
...
addParameter(p,'IfFindConnectivity',default_param.IfFindConnectivity,@islogical);
